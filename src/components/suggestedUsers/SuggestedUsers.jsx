...
console.log(user);
  useEffect(() => {
    const fetchSuggestedUsers = async() => {
      try {
         const res = await fetch(`${API}/user/find/suggestedUsers`, {
          headers: {
  ...
  const toggleFollow = async(id) => {
    try {
      await fetch(`${API}/user/toggleFollow/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        },
        method: "PUT"
    ...
    <div className={classes.container}>
      <div className={classes.wrapper}>
        <div className={classes.myProfile}>
          <img src={`${API}/images/${user.profileImg}`} className={classes.profileUserImg} alt=""/>
              ...
              {suggestedUsers?.map((suggestedUser) => (
                <div className={classes.suggestedUser} key={suggestedUser._id}>
                  <Link to={`/profileDetail/${suggestedUser._id}`}>
                    <img src={`${API}/images/${suggestedUser.profileImg}`} className={classes.imgUser}/>
                  </Link> 
